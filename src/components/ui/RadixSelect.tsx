...
interface RadixSelectOption { label: string; value: string }
interface RadixSelectProps {
  value?: string;
  onChange: (v: string) => void;
  options: Array<string | RadixSelectOption>;
  placeholder?: string;
  className?: string;
}

const RadixSelect: React.FC<RadixSelectProps> = ({ value, onChange, options, placeholder = "Sélectionner", className }) => {
  const normalized: RadixSelectOption[] = React.useMemo(() => options.map((o) => (typeof o === 'string' ? { label: o, value: o } : o)), [options]);
  return (
      ...
      <Select.Portal>
        <Select.Content className="overflow-hidden bg-white border rounded-lg shadow-xl">
          <Select.Viewport className="p-1">
            {normalized.map((opt) => (
              <Select.Item key={opt.value} value={opt.value} className="flex items-center justify-between px-3 py-2 rounded hover:bg-gray-100 cursor-pointer">
